...
APP_NAME = "ReflectorTracker"
APP_VERSION = "5.31"
IR_MODEL_VERSION = 14
IR_SEARCH_SCALE = 1.0
IR_STRICT_REGIONS = True
IR_GLOBAL_FALLBACK = False
...
def configure_logging() -> None:
    """Настраивает логирование только при запуске приложения как программы."""
    logging.basicConfig(
        level=logging.INFO,
...
PRESET_FIELDS = (
    ...
    "lost_hold_frames",
    "max_jump",
    "ir_stationary_deadband",
    "ir_reacquire_confirm_frames",
    "ir_reacquire_consistency",
    "ir_template_min_score",
)
...
# Базовый вид окна: только два точных крестика — текущий центр
# и нулевое (стартовое) положение. Все остальные элементы — отдельные
# независимые слои.
BASE_DISPLAY_SETTINGS = {
    "show_points": True,
    "show_start_points": True,
    "show_labels": False,
    "show_bright_contours": False,
    "show_circles": False,
    "show_frames": False,
    "show_lines": False,
    "show_distances": False,
    "show_distance_changes": False,
    "show_displacement_lines": False,
...
BUILTIN_PRESETS = {
    "Базовый": {
        ...
        "lost_hold_frames": 75,
        "max_jump": 9.0,
        "ir_stationary_deadband": 0.50,
        "ir_reacquire_confirm_frames": 3,
        "ir_reacquire_consistency": 5.0,
        "ir_template_min_score": 0.32,
    },
    "Фиксированный точный": {
        ...
        "max_jump": 5.0,
        "ir_stationary_deadband": 0.35,
        "ir_reacquire_confirm_frames": 4,
        "ir_reacquire_consistency": 4.0,
        "ir_template_min_score": 0.38,
    },
    "Широкий блик": {
        "min_area": 5,
        "max_area": 700,
        ...
        "lost_hold_frames": 90,
        "max_jump": 10.0,
        "ir_stationary_deadband": 0.65,
        "ir_reacquire_confirm_frames": 3,
        "ir_reacquire_consistency": 7.0,
        "ir_template_min_score": 0.28,
    },
}
...
def bounding_rect(regions: List[Rect]) -> Optional[Rect]:
    """Возвращает минимальную общую рамку набора прямоугольных областей."""
    if not regions:
        return None
...
def merge_ir_candidates(
    raw_candidates: List[Dict],
    bbox_key: str,
    minimum_distance: float,
) -> List[Dict]:
    """Удаляет дубликаты ИК-кандидатов, сохраняя прежний порядок приоритетов."""
    merged: List[Dict] = []
    for candidate in sorted(
        ...
        duplicate = None
        candidate_hints = set(candidate.get("region_hints", set()))
        for existing in merged:
            if candidate_hints.isdisjoint(
                set(existing.get("region_hints", set()))
            ):
                continue
            distance = float(
                np.linalg.norm(
            ...
            merge_distance = max(
                ...
                0.25
                * min(
                    max(candidate[bbox_key][2:]),
                    max(existing[bbox_key][2:]),
                ),
            )
...
def normalized_reflector_patch(
    image: np.ndarray,
    center: Point,
    size: int = 25,
) -> np.ndarray:
    """Возвращает нормированный локальный рисунок яркого ромба.

    Абсолютная яркость намеренно удаляется: экспозиция RTSP меняется, а
    пространственный рисунок ядра и ореола остается признаком именно этого
    отражателя. Равномерная белая поверхность дает почти нулевой шаблон.
    """
    patch_size = max(9, int(round(size)) | 1)
    if image.ndim == 3:
    ...
    background = float(np.median(border)) if border.size else float(np.median(patch))
    signal = np.maximum(patch - background, 0.0)
    upper = float(np.percentile(signal, 98.0))
    if upper <= 1e-6:
        return np.zeros((patch_size, patch_size), dtype=np.float32)
    signal = np.clip(signal / upper, 0.0, 1.0)
    # Слабый ореол полезен для ромба, но не должен перевешивать ядро.
    signal = np.sqrt(signal, dtype=np.float32)
    return signal.astype(np.float32)


def reflector_template_score(reference, current: np.ndarray) -> float:
    """Косинусное сходство двух нормированных локальных шаблонов."""
    try:
        reference_array = np.asarray(reference, dtype=np.float32)
        current_array = np.asarray(current, dtype=np.float32)
    except (TypeError, ValueError):
        return 0.0
    if reference_array.size == 0 or reference_array.shape != current_array.shape:
        return 0.0
    reference_flat = reference_array.reshape(-1)
    current_flat = current_array.reshape(-1)
    denominator = float(
        np.linalg.norm(reference_flat) * np.linalg.norm(current_flat)
    )
    if denominator <= 1e-8:
        return 0.0
    return float(
        np.clip(np.dot(reference_flat, current_flat) / denominator, 0.0, 1.0)
    )


def locate_reflector_by_template(
    image: np.ndarray,
    predicted_center: Point,
    reference_template,
    template_size: int = 25,
    search_radius: float = 12.0,
    subpixel_step: float = 0.5,
) -> Tuple[Optional[Point], float]:
    """Находит сдвиг целого ромба, а не центр меняющейся яркой маски.

    Кандидаты проверяются в локальном окне около предыдущей позиции. Одинаково
    обрабатываются ядро и четыре луча ореола, поэтому изменение экспозиции не
    заставляет маркер метаться между левым и правым краями отражателя.
    """
    try:
        reference = np.asarray(reference_template, dtype=np.float32)
    except (TypeError, ValueError):
        return None, 0.0
    size = max(9, int(round(template_size)) | 1)
    if reference.size != size**2:
        return None, 0.0
    reference = reference.reshape(size, size)
    if float(np.linalg.norm(reference)) <= 1e-8:
        return None, 0.0

    # Один matchTemplate заменяет сотни повторных сегментаций и не зависит от
    # того, какой луч ореола оказался насыщен сильнее на текущем кадре.
    if image.ndim == 3:
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY).astype(np.float32)
    else:
        gray = np.asarray(image, dtype=np.float32)
    radius = max(1, int(np.ceil(float(search_radius))))
    search_size = size + 2 * radius
    search_patch = cv2.getRectSubPix(
        gray,
        (search_size, search_size),
        (float(predicted_center[0]), float(predicted_center[1])),
    )
    search_background = float(np.median(search_patch))
    search_patch = np.sqrt(
        np.maximum(search_patch - search_background, 0.0),
        dtype=np.float32,
    )
    ...
    if response.size == 0 or not np.any(np.isfinite(response)):
        return None, 0.0
    ...
    # Параболическая субпиксельная поправка по трем значениям корреляции.
    def subpixel_offset(before: float, center: float, after: float) -> float:
        denominator = before - 2.0 * center + after
        if abs(denominator) <= 1e-8:
            return 0.0
        value = 0.5 * (before - after) / denominator
        limit = max(0.0, min(1.0, float(subpixel_step)))
    ...
    if 0 < peak_x < response.shape[1] - 1:
        offset_x = subpixel_offset(
            float(response[peak_y, peak_x - 1]),
            float(response[peak_y, peak_x]),
            float(response[peak_y, peak_x + 1]),
        )
    if 0 < peak_y < response.shape[0] - 1:
        offset_y = subpixel_offset(
            float(response[peak_y - 1, peak_x]),
            float(response[peak_y, peak_x]),
            float(response[peak_y + 1, peak_x]),
        )
    center = (
        float(predicted_center[0] + peak_x - radius + offset_x),
        float(predicted_center[1] + peak_y - radius + offset_y),
    )
    return center, float(np.clip(maximum, 0.0, 1.0))


def detector_center_to_marker_center(
    detector_position: Point,
    tracking_offset: Point,
) -> Point:
    """Переводит центр Night-детектора в координату Day↔Night-маркера.

    Day↔Night-центр является опорным и никогда не заменяется повторной
    сегментацией. Служебное смещение двух способов измерения вычитается из
    каждого рабочего измерения; физическое перемещение сохраняется.
    """
    return (
        float(detector_position[0] - tracking_offset[0]),
        float(detector_position[1] - tracking_offset[1]),
    )


def update_tracking_offset_bootstrap(
    samples,
    raw_offset: Point,
    tolerance: float = 5.0,
    required: int = 3,
) -> Tuple[List[List[float]], Optional[Point]]:
    """Накопляет согласованные поправки рабочего Night-центра."""
    parsed_samples = []
    for sample in samples or []:
        try:
            value = np.asarray(sample, dtype=np.float64).reshape(2)
        except (TypeError, ValueError):
            continue
        if np.all(np.isfinite(value)):
            parsed_samples.append(value)
    current = np.asarray(raw_offset, dtype=np.float64).reshape(2)
    if parsed_samples:
        median_offset = np.median(np.stack(parsed_samples), axis=0)
        if float(np.linalg.norm(current - median_offset)) > max(0.5, tolerance):
            parsed_samples = []
    parsed_samples.append(current)
    parsed_samples = parsed_samples[-max(1, int(required)) :]
    serialized = [value.tolist() for value in parsed_samples]
    if len(parsed_samples) < max(1, int(required)):
        return serialized, None
    calibrated = np.median(np.stack(parsed_samples), axis=0)
    return serialized, (float(calibrated[0]), float(calibrated[1]))
...
def assign_ir_candidates_to_regions(
    ...
    for candidate_index, candidate in enumerate(candidates):
        ...
        for region_index in range(len(reference_regions)):
            ...
            if strict_hints and region_index not in hints:
...
class PeakDetection:
    ...
    # Точный контур итоговой яркой маски, из которой детектор
    # получил центр. Координаты уже переведены в полный кадр.
    measurement_contours: Tuple[np.ndarray, ...] = ()
...
class ReflectorPoint:
    ...
    threshold: float = 0.0
    # Контуры только последнего принятого измерения. В HOLD/LOST слой
    # очищается, чтобы старая маска не выглядела как текущая.
    measurement_contours: Tuple[np.ndarray, ...] = ()
    ...
    # Причина, почему последний кадр не дал измерения.
    # Выводится в HOLD/LOST и позволяет отличить слабый сигнал
    # от выхода за допуск перемещения.
    rejection_reason: str = ""
    # Перезахват после HOLD не переносит маркер за один кадр.
    # Кандидат должен сформировать короткую согласованную траекторию.
    pending_position: Optional[Point] = None
    pending_velocity: Point = (0.0, 0.0)
...
class CalibrationData:
    ...
    def __init__(self):
        ...
        # Day↔Night-калибровка всегда ищет ровно внутри
        # ручной рамки Pn. Поле оставлено в JSON только для
        # совместимости со старыми пресетами.
        self.ir_search_scale = IR_SEARCH_SCALE
        ...
        self.ir_strict_regions = IR_STRICT_REGIONS
        # Глобальный fallback запрещен: объект из-за рамки Pn
        # не имеет права участвовать в ее Day↔Night-калибровке.
        self.ir_global_fallback = IR_GLOBAL_FALLBACK
        ...
        # Мелкое изменение субпиксельного центра при неизменном
        # отражателе считается шумом экспозици/сжатия.
        self.ir_stationary_deadband = 0.50
        # После потери крупный перенос принимается только после
        # нескольких последовательных согласованных кадров.
        self.ir_reacquire_confirm_frames = 3
        self.ir_reacquire_consistency = 5.0
        self.ir_template_min_score = 0.32
        ...
        self.ir_model_version = IR_MODEL_VERSION
    ...
    def _enforce_ir_region_invariants(self) -> None:
        """Фиксирует режим строгой Day↔Night-привязки внутри ручной Pn."""
        self.ir_search_scale = IR_SEARCH_SCALE
        self.ir_strict_regions = IR_STRICT_REGIONS
        self.ir_global_fallback = IR_GLOBAL_FALLBACK
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "show_start_points": self.show_start_points,
            "show_labels": self.show_labels,
            "show_bright_contours": self.show_bright_contours,
            ...
            "show_displacement_lines": self.show_displacement_lines,
            ...
            "ir_stationary_deadband": self.ir_stationary_deadband,
            "ir_reacquire_confirm_frames": self.ir_reacquire_confirm_frames,
            "ir_reacquire_consistency": self.ir_reacquire_consistency,
            "ir_template_min_score": self.ir_template_min_score,
    ...
    def load(self, filename: str):
        ...
        # В файлах v5.30 красная точка, подпись и числа были сшиты в
        # два переключателя. При первой загрузке такого JSON переходим на
        # новый базовый вид из двух крестиков. Новые JSON хранят все слои.
        if "show_start_points" not in data:
            for name, value in BASE_DISPLAY_SETTINGS.items():
                setattr(self, name, value)

        # Старый JSON не должен вернуть ошибочный поиск за
        # границами Pn. Эти три значения теперь инварианты.
        self._enforce_ir_region_invariants()
        ...
        self.ir_stationary_deadband = float(
            np.clip(float(self.ir_stationary_deadband), 0.0, 2.0)
        )
        self.ir_reacquire_confirm_frames = int(
            np.clip(int(self.ir_reacquire_confirm_frames), 1, 10)
        )
        self.ir_reacquire_consistency = float(
            np.clip(float(self.ir_reacquire_consistency), 1.0, 20.0)
        )
        self.ir_template_min_score = float(
            np.clip(float(self.ir_template_min_score), 0.0, 0.95)
        )
        ...
        if reference_regions and len(reference_regions) == len(self.peak_regions):
            ...
            self.roi_region = bounding_rect(self.peak_regions)
            ...
            for raw_id, raw_model in raw_models.items():
                ...
                for name in (
                    ...
                    if name not in raw_model:
                        continue
                    try:
                        value = float(raw_model[name])
                    except (TypeError, ValueError):
                        continue
                    if np.isfinite(value):
                        cleaned_model[name] = value

                raw_offset_valid = raw_model.get("tracking_offset_valid")
                if isinstance(raw_offset_valid, (bool, np.bool_)):
                    cleaned_model["tracking_offset_valid"] = bool(
                        raw_offset_valid
                    )
                elif isinstance(raw_offset_valid, (int, np.integer)) and int(
                    raw_offset_valid
                ) in (0, 1):
                    cleaned_model["tracking_offset_valid"] = bool(
                        raw_offset_valid
                ...
                try:
                    template_size = int(
                        raw_model.get("tracking_template_size", 0)
                    )
                except (TypeError, ValueError):
                    template_size = 0
                if 9 <= template_size <= 101 and template_size % 2 == 1:
                    cleaned_model["tracking_template_size"] = template_size
                    raw_template = raw_model.get("tracking_template")
                    if raw_template is not None:
                        try:
                            template = np.asarray(
                                raw_template, dtype=np.float32
                            )
                        except (TypeError, ValueError):
                            template = np.empty((0, 0), dtype=np.float32)
                        if (
                            template.shape == (template_size, template_size)
                            and np.all(np.isfinite(template))
                        ):
                            cleaned_model["tracking_template"] = np.clip(
                                template, 0.0, 1.0
                            ).tolist()
        ...
        try:
            ...
            with self.condition:
                if not self.closed:
                    ...
                    self.last_error = (
                        "RTSP-поток FFmpeg завершился"
                        if return_code is None
                        else f"RTSP-поток FFmpeg завершился, код {return_code}"
                    )
                self.is_connected = False
                self.condition.notify_all()
        ...
        if self.process is not None and self.process.poll() is None:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning("Не удалось дождаться завершения FFmpeg RTSP")
        with self.condition:
            self.condition.notify_all()
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.warning(
                                "Не удалось дождаться завершения FFmpeg записи"
                            )
            self.is_recording = False
...
class ReflectorDetector:
    ...
    def detect_in_region(
        ...
        if rect is None:
            self.last_rejection_reason = "REGION"
            return None
        ...
        if anchor is not None:
            ...
            if probe_x2 > probe_x1 and probe_y2 > probe_y1:
                ...
                # Для подтвержденного отражателя фон берем из
                # кольца вокруг последней точки, а не с границы всей
                # рамки, где может преобладать белая крыша.
                probe_y, probe_x = np.indices(signal.shape, dtype=np.float32)
                probe_distance = np.sqrt(
                    (probe_x - anchor_probe_local[0]) ** 2
                    + (probe_y - anchor_probe_local[1]) ** 2
                )
                inner_radius = max(4.0, min(10.0, probe_radius * 0.32))
                local_ring = signal_float[
                    (probe_distance >= inner_radius)
                    & (probe_distance <= probe_radius)
                ]
                if local_ring.size >= 12:
                    background_level = float(np.median(local_ring))
        ...
        anchor_ok = (
            anchor is not None
            and detection_peak >= max(60.0, brightness * 0.38)
            and detection_contrast >= max(4.0, contrast * 0.22)
        )
        required_contrast = max(4.0, contrast * 0.22) if anchor_ok else contrast
        if (
            not (absolute_ok or adaptive_ok or anchor_ok)
            or detection_contrast < required_contrast
        ):
            self.last_rejection_reason = "LOW_SIGNAL"
            return None
        ...
        if anchor_ok:
            ...
            relative_core_level = background_level + 0.52 * detection_contrast
            peak_core_level = detection_peak - max(
                18.0, 0.42 * detection_contrast
            )
            threshold = max(
        ...
        if count <= 1:
            self.last_rejection_reason = "NO_CORE"
            return None
        ...
        if anchor_probe_local is not None:
            # На одном RTSP-кадре насыщенное ядро может
            # сжаться до одного пикселя. После двух циклов Day↔Night это
            # не является причиной терять уже подтвержденную метку.
            min_area = 1
        ...
        if anchor_model:
            ...
            try:
                ...
                response_anchor_diamond = 0.0
            try:
                tracking_template_size = max(
                    9, int(anchor_model.get("tracking_template_size", 25)) | 1
                )
                raw_template = anchor_model.get("tracking_template")
                if raw_template is not None:
                    parsed_template = np.asarray(raw_template, dtype=np.float32)
                    if parsed_template.size == tracking_template_size**2:
                        tracking_template = parsed_template.reshape(
                            tracking_template_size, tracking_template_size
                        )
            except (TypeError, ValueError):
                tracking_template = None
        if expected_anchor_area > 0:
            ...
            max_area = max(max_area, int(np.ceil(expected_anchor_area * 12.0)))
        ...
        for label in range(1, count):
            ...
            if expected_anchor_area > 0:
                ...
                # Площадь ореола и его ромбовидность на одном кадре
                # нестабильны из-за экспозици и H.264. Они дают вес,
                # но больше не удаляют яркое ядро в верном месте.
                area_similarity = float(
                    np.exp(-abs(np.log(max(area_ratio, 1e-6))) / 1.15)
                )
            component_template_score = 0.0
            if tracking_template is not None:
                component_template_score = reflector_template_score(
                    tracking_template,
                    normalized_reflector_patch(
                        frame,
                        (float(center[0] + x0), float(center[1] + y0)),
            ...
            if anchor_local is not None:
                component_peak = float(np.max(signal_float[component_mask]))
                peak_similarity = float(
                    np.clip(
                        (component_peak - background_level)
                        / max(1.0, detection_contrast),
                        0.0,
                        1.0,
                    )
                )
                # Главный признак после калибровки — непрерывность
                # координаты. Форма/площадь — только мягкие добавки.
                score = (
                    0.48 * anchor_proximity
                    + 0.14 * proximity
                    + 0.09 * peak_similarity
                    + 0.07 * area_similarity
                    + 0.07 * diamond_similarity
                    + 0.15 * component_template_score
                )
            else:
        ...
        while changed:
            changed = False
            for _, label, bbox, _, _, _ in candidates:
        ...
        if final_area < min_area:
            self.last_rejection_reason = "CORE_AREA"
            return None
        ...
        if tracking_template is not None:
            best_template_score = reflector_template_score(
                tracking_template,
                normalized_reflector_patch(
                    frame,
        ...
        # Храним все фрагменты итоговой маски. Это именно те пиксели,
        # которые участвовали в вычислении центра, а не условная рамка.
        measurement_contours = []
        contour_offset = np.array([[[x0, y0]]], dtype=np.int32)
        for contour in contours:
            if contour.size:
                measurement_contours.append(
                    contour.astype(np.int32, copy=True) + contour_offset
                )
        circularity = 0.0
        if contours:
        ...
        return PeakDetection(
            ...
            template_score=float(best_template_score),
            measurement_contours=tuple(measurement_contours),
        )
...
class ReflectorTracker:
    ...
    def reset_regions(self):
        ...
        for index, region in enumerate(self.calibration.peak_regions):
            ...
            track = ReflectorPoint(
                ...
                # Для подтвержденной ИК-точки ноль задается результатом
                # Day↔Night, а не первым нестабильным кадром сегментации.
                initial_position=center if has_verified_anchor else None,
            )
    ...
    def update_settings(self):
        current_signature = tuple(
            ...
            for region in self.calibration.peak_regions
            if int(region[2]) >= 3 and int(region[3]) >= 3
        )
    ...
    @staticmethod
    def _clear_pending_reacquisition(track: ReflectorPoint):
        track.pending_position = None
        track.pending_velocity = (0.0, 0.0)
        track.pending_frames = 0

    def _confirm_reacquisition(
        self, track: ReflectorPoint, candidate: Point
    ) -> bool:
        """Подтверждает перезахват короткой согласованной траекторией.

        Первый кандидат только запоминается. Это исключает прежний сценарий,
        когда единственный кадр с тенью/бликом мгновенно переносил P1. При этом
        реальный отражатель может продолжать двигаться: со второго кадра
        сравнение идет с прогнозом скорости кандидата.
        """
        required = max(1, int(self.calibration.ir_reacquire_confirm_frames))
        candidate_array = np.asarray(candidate, dtype=np.float64)
        if required <= 1:
            self._clear_pending_reacquisition(track)
            return True

        if track.pending_position is None:
            track.pending_position = (
                float(candidate_array[0]),
                float(candidate_array[1]),
            )
            track.pending_velocity = (0.0, 0.0)
            track.pending_frames = 1
            return False

        previous = np.asarray(track.pending_position, dtype=np.float64)
        velocity = np.asarray(track.pending_velocity, dtype=np.float64)
        prediction = previous + velocity
        tolerance = max(
            1.0, float(self.calibration.ir_reacquire_consistency)
        )
        if track.pending_frames == 1:
            # Для реально движущейся метки скорость еще неизвестна.
            tolerance = max(
                tolerance,
                min(24.0, max(12.0, float(self.calibration.max_jump) * 2.0)),
            )
        residual = float(np.linalg.norm(candidate_array - prediction))
        if residual > tolerance:
            # Несогласованный объект начинает подтверждение заново и не
            # влияет на действующий HOLD-центр.
            track.pending_position = (
                float(candidate_array[0]),
                float(candidate_array[1]),
            )
        ...
        measured_velocity = candidate_array - previous
        filtered_velocity = 0.45 * velocity + 0.55 * measured_velocity
        track.pending_position = (
            float(candidate_array[0]),
            float(candidate_array[1]),
        )
        track.pending_velocity = (
            float(filtered_velocity[0]),
            float(filtered_velocity[1]),
        )
        track.pending_frames += 1
        if track.pending_frames < required:
            return False
        self._clear_pending_reacquisition(track)
        return True
    ...
    def _update_track(
        ...
        ir_tracking_offset_valid = False
        ...
        if self.calibration.ir_lock_enabled:
            ...
            if saved_anchor and len(saved_anchor) == 2:
                ...
                if ir_anchor_model:
                    ir_tracking_offset_valid = bool(
                        ir_anchor_model.get("tracking_offset_valid", False)
                    )
                    try:
                        if ir_tracking_offset_valid:
                ...
                maximum_offset = max(
                    3.0,
                    min(12.0, float(self.calibration.ir_lock_radius)),
                )
        ...
        if needs_ir_confirmation:
            ...
            if detection is None:
                rejection_reason = (
                    self.detector.last_rejection_reason or "NO_CORE"
                )
            if detection is not None and ir_anchor is not None:
                # Детектор Night и Day→Night могут иметь постоянное смещение
                # центра из-за разной бинаризации одного ореола. Правильный
                # Day↔Night-центр не заменяем: служебную разницу вычитаем из
                # каждого рабочего измерения. Если сохраненная поправка
                # отсутствует (например, модель v5.28 с NO_OFFSET), обучаем
                # ее на трех стабильных рабочих кадрах, не двигая маркер.
                if not ir_tracking_offset_valid:
                    if ir_anchor_model is None:
                        ir_anchor_model = {}
                        self.calibration.ir_confirmed_models[
                            str(track.id)
                        ] = ir_anchor_model
                    has_template = (
                        ir_anchor_model.get("tracking_template") is not None
                    )
                    startup_template_min = max(
                        0.08,
                        min(
                            0.18,
                            float(self.calibration.ir_template_min_score) * 0.55,
                        ),
                    )
                    if (
                        has_template
                        and detection.template_score < startup_template_min
                    ):
                        ir_anchor_model.pop("_offset_bootstrap_samples", None)
                        detection = None
                        rejection_reason = "OFFSET_TEMPLATE"
                    else:
                        raw_offset = (
                            np.asarray(detection.position, dtype=np.float64)
                            - np.asarray(ir_anchor, dtype=np.float64)
                        )
                        offset_limit = max(
                            3.0,
                            min(12.0, float(self.calibration.ir_lock_radius)),
                        )
                        if float(np.linalg.norm(raw_offset)) > offset_limit:
                            ir_anchor_model.pop(
                                "_offset_bootstrap_samples", None
                            )
                            detection = None
                            rejection_reason = "OFFSET_RANGE"
                        else:
                            samples = ir_anchor_model.setdefault(
                                "_offset_bootstrap_samples", []
                            )
                            tolerance = max(
                                2.0,
                                min(
                                    5.0,
                                    float(
                                        self.calibration.ir_reacquire_consistency
                                    ),
                                ),
                            )
                            bootstrap_samples, calibrated_offset = (
                                update_tracking_offset_bootstrap(
                                    samples,
                                    (
                                        float(raw_offset[0]),
                                        float(raw_offset[1]),
                            ...
                            if calibrated_offset is None:
                                detection = None
                                rejection_reason = (
                                    f"OFFSET_CAL_{len(bootstrap_samples)}/3"
                                )
                            else:
                                ir_tracking_offset = np.asarray(
                                    calibrated_offset, dtype=np.float64
                                )
                                ir_anchor_model[
                                    "tracking_center_offset_x"
                                ] = float(ir_tracking_offset[0])
                                ir_anchor_model[
                                    "tracking_center_offset_y"
                                ] = float(ir_tracking_offset[1])
                                ir_anchor_model["tracking_offset_valid"] = True
                                ir_anchor_model.pop(
                                    "_offset_bootstrap_samples", None
                                )
                                ir_tracking_offset_valid = True
                                # Это не потеря и не перезахват: три кадра
                                # подтвердили тот же неподвижный отражатель.
                                track.missed_frames = 0
                if detection is not None and ir_tracking_offset_valid:
                    detection.position = detector_center_to_marker_center(
                        detection.position,
                        (
                            float(ir_tracking_offset[0]),
                            float(ir_tracking_offset[1]),
                    ...
                    # Координата сегментированной маски используется только
                    # для грубой проверки. Точный сдвиг измеряем корреляцией
                    # всего сохраненного ромба относительно предыдущей точки.
                    raw_template = (
                        ir_anchor_model.get("tracking_template")
                        if ir_anchor_model
                        else None
                    )
                    if raw_template is not None:
                        template_size = int(
                            ir_anchor_model.get("tracking_template_size", 25)
                        )
                        detector_template_center, template_match = (
                            locate_reflector_by_template(
                                frame,
                                predicted_center=(
                                    float(track.position[0] + ir_tracking_offset[0]),
                                    float(track.position[1] + ir_tracking_offset[1]),
                                ),
                                reference_template=raw_template,
                                template_size=template_size,
                                search_radius=max(
                                    2.0, float(ir_anchor_radius or 0.0)
                                ),
                                subpixel_step=0.5,
                            )
                        )
                        # На старте достаточно умеренного сходства: яркость
                        # Night-кадра меняется, но положение целого рисунка
                        # надежнее положения отдельного насыщенного пикселя.
                        template_acceptance = max(
                            0.12,
                            min(
                                0.30,
                                float(self.calibration.ir_template_min_score)
                                * 0.80,
                            ),
                        )
                        if (
                            detector_template_center is not None
                            and template_match >= template_acceptance
                        ):
                            detection.position = detector_center_to_marker_center(
                                detector_template_center,
                                (
                                    float(ir_tracking_offset[0]),
                                    float(ir_tracking_offset[1]),
                                ),
                            )
                            detection.template_score = template_match
                        else:
                            detection = None
                            rejection_reason = "TEMPLATE_CENTER"
        ...
        if detection is not None and track.has_measurement:
            ...
            if ir_anchor is not None:
                # Детектор уже ограничил кандидата динамическим
                # радиусом вокруг последней точки. Второй, меньший
                # max_jump раньше удалял уже прошедший кандидат.
                allowed_jump = max(allowed_jump, float(ir_anchor_radius or 0.0))
            if track.missed_frames > 0:
                allowed_jump = max(
                    allowed_jump,
                    0.5 * float(np.hypot(search_region[2], search_region[3])),
                )
                if track.detection_area > 0 and ir_anchor is None:
                    ...
                    if (
                        ...
                        detection = None
                        rejection_reason = "AREA_JUMP"
            if detection is not None and jump > allowed_jump:
                detection = None
                rejection_reason = "MAX_JUMP"

        # После HOLD новый яркий объект не имеет права мгновенно перенести
        # маркер. Сначала проверяем рисунок того же Night-ромба, затем требуем
        # несколько согласованных кадров. Логика одинакова для всех Pn.
        reacquired_this_frame = False
        if (
            detection is not None
            and ir_anchor is not None
            and track.has_measurement
            and track.missed_frames > 0
        ):
            has_template = bool(
                ir_anchor_model
                and ir_anchor_model.get("tracking_template") is not None
            )
            if has_template and detection.template_score < float(
                self.calibration.ir_template_min_score
            ):
                detection = None
                rejection_reason = "TEMPLATE"
                self._clear_pending_reacquisition(track)
            else:
                reacquired_this_frame = self._confirm_reacquisition(
                    track, detection.position
                )
                if not reacquired_this_frame:
                    pending_count = int(track.pending_frames)
                    required_count = max(
                        1, int(self.calibration.ir_reacquire_confirm_frames)
                    )
                    detection = None
                    rejection_reason = (
                        f"REACQUIRE_{pending_count}/{required_count}"
                    )
        ...
        if detection is not None:
            ...
            if (
                ir_anchor is not None
                and track.has_measurement
                and track.missed_frames <= 0
                and float(np.linalg.norm(measurement - old_position))
                <= float(self.calibration.ir_stationary_deadband)
            ):
                # Субпиксельная миграция центра из-за H.264/экспозиции не
                # превращается в физическое перемещение отражателя.
                measurement = old_position.copy()
            if (
                not track.has_measurement
                or not track.is_active
                or track.missed_frames > 5
                or reacquired_this_frame
            ):
            ...
            track.measurement_contours = tuple(
                contour.copy() for contour in detection.measurement_contours
            )
            ...
            if displacement_magnitude <= float(
                self.calibration.ir_stationary_deadband
            ):
                displacement_x = 0.0
                displacement_y = 0.0
                displacement_magnitude = 0.0
            track.displacement = (displacement_x, displacement_y)
            track.displacement_magnitude = displacement_magnitude
            ...
            # Диагностический слой должен показывать только то, что
            # детектор принял на этом кадре. В HOLD/LOST старый контур
            # не сохраняем.
            track.measurement_contours = ()
            track.rejection_reason = rejection_reason or "NO_CORE"
            if not track.rejection_reason.startswith("REACQUIRE_"):
                self._clear_pending_reacquisition(track)
    ...
    def _visualize(self, frame: np.ndarray, quality: Dict) -> np.ndarray:
        ...
        for track in self.tracks:
            ...
            if (
                calibration.show_bright_contours
                and track.measured_this_frame
                and track.measurement_contours
            ):
                # Бирюзовый контур — точная итоговая маска центра.
                # Он рисуется до маркеров, чтобы крестик оставался читаемым.
                cv2.drawContours(
                    display,
                    list(track.measurement_contours),
                    -1,
                    (255, 220, 0),
                    1,
                    cv2.LINE_AA,
            ...
            if track.has_measurement:
                ...
                if track.measured_this_frame:
                    ...
                    state = f"HOLD {track.missed_frames}:{track.rejection_reason}"
                else:
                    point_color = (0, 0, 255)
                    state = f"LOST:{track.rejection_reason}"
                ...
                if calibration.show_points:
                    # Текущий центр — красный наклонный крестик. Он не
                    # закрывает собой центральные пиксели отражателя.
                    cv2.drawMarker(
                        display,
                        (px, py),
                        (0, 0, 255),
                ...
                if track.initial_position is not None:
                    ...
                    if calibration.show_start_points:
                        # Старт — прямой пурпурный крестик. Другая форма и цвет
                        # позволяют отличить его от текущей координаты.
                        cv2.drawMarker(
                            display,
                            (initial_x, initial_y),
                            displacement_color,
                    ...
                    if (
                        calibration.show_displacement_lines
                        and (initial_x != px or initial_y != py)
                    ):
                        cv2.line(
                            display,
                            (initial_x, initial_y),
                    ...
                    if calibration.show_displacements:
                        label_x = max(
            ...
            elif calibration.show_labels and not calibration.show_frames:
                outlined_text(
                    f"P{track.id} SEARCH",
                    (bx, max(15, by - 5)),
                    0.45,
...
class ReflectorApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title(f"Система устойчивого отслеживания отражателей v{APP_VERSION}")
    ...
    def _create_layer_drawer(self):
        ...
        layer_options = (
            ("Текущие центры (крестики)", self.show_points_var),
            ("Стартовые центры (крестики)", self.show_start_points_var),
            ("Подписи Pn / LOCK / HOLD", self.show_labels_var),
            ("Яркая маска центра (контур)", self.show_bright_contours_var),
            ...
            ("Линии от старта к центру", self.show_displacement_lines_var),
            ("Числа смещения dX/dY/dR", self.show_displacements_var),
            ("Общая строка Peaks", self.show_summary_var),
        ...
        ToolTip(
            self.layer_drawer,
            "Слои независимы. Бирюзовый контур показывает точную "
            "яркую маску, из которой детектор вычислил центр.",
        )
    ...
    def _create_widgets(self):
        ...
        self.ir_search_scale_var = tk.DoubleVar(value=1.0)
        ...
        ttk.Label(flash_row, text="Граница Pn (строго):").pack(side=tk.LEFT)
        ttk.Spinbox(
            flash_row,
            from_=1.0,
            to=12.0,
        ...
        self.stationary_deadband_var = tk.DoubleVar(
            value=self.calibration.ir_stationary_deadband
        )
        self.reacquire_frames_var = tk.IntVar(
            value=self.calibration.ir_reacquire_confirm_frames
        )
        self.reacquire_consistency_var = tk.DoubleVar(
            value=self.calibration.ir_reacquire_consistency
        )
        self.template_min_score_var = tk.DoubleVar(
            value=self.calibration.ir_template_min_score
        )
        self._add_scale(
            tracking,
        ...
        self._add_scale(
            tracking,
            "Мёртвая зона неподвижной точки, px",
            self.stationary_deadband_var,
            0.0,
            2.0,
            2,
            "Подавляет субпиксельное дрожание от экспозиции и H.264.",
        )
        self._add_scale(
            tracking,
            "Подтверждение перезахвата, кадров",
            self.reacquire_frames_var,
            1,
            10,
            0,
            "После HOLD новая точка должна повториться несколько кадров.",
        )
        self._add_scale(
            tracking,
            "Допуск траектории перезахвата, px",
            self.reacquire_consistency_var,
            1.0,
            20.0,
            1,
        )
        self._add_scale(
            tracking,
            "Мин. сходство Night-шаблона",
            self.template_min_score_var,
            0.0,
            0.95,
            2,
            "Отсекает соседний яркий объект после потери отражателя.",
        )
        ...
        self.show_start_points_var = tk.BooleanVar(
            value=self.calibration.show_start_points
        )
        self.show_labels_var = tk.BooleanVar(value=self.calibration.show_labels)
        self.show_bright_contours_var = tk.BooleanVar(
            value=self.calibration.show_bright_contours
        )
        ...
        self.show_displacement_lines_var = tk.BooleanVar(
            value=self.calibration.show_displacement_lines
        )
        self.show_displacements_var = tk.BooleanVar(
            value=self.calibration.show_displacements
        )
        self.show_summary_var = tk.BooleanVar(value=self.calibration.show_summary)
...
Базовый вид — два крестика: красный наклонный показывает текущий центр,
пурпурный прямой — стартовое положение. Бирюзовый контур «Яркая маска центра»
обводит именно те пиксели, по которым детектор вычислил центр. Все крестики,
подписи, маска, круги, рамки, линии и числа включаются независимо.
Точки соединяются по порядку P1—P2—P3...
...
1. Точно обведите каждый отражатель его собственной рамкой
   P1…Pn и запустите живой RTSP-поток. Граница рамки жесткая:
   во время Day↔Night объект снаружи не участвует в поиске.
...
4. Зеленые метки — подтвержденные ИК-отражатели. D — сходство светового
   ореола с ромбом. Оранжевые рамки — кандидаты только своей Pn.
   S — итоговая достоверность Day/Night. P1 не видит кандидатов P2
   и наоборот. Центр уточняется по яркому Night-ядру, но уточнитель
   также жестко обрезан своей рамкой.
   ...
   X:OUTSIDE_REGION/X:FOREIGN_REGION — кандидат не принадлежит этой рамке.
5. Локальный режим всегда включен, а поиск вне рамок всегда отключен.
   Каждая Pn получает собственный порог. Рекомендуемое «Мин. D» — 0,40–0,55.
   Если отражатель не найден, уменьшите
   ...
   Черное Day-ядро ищется отдельно вокруг Night-ромба с локальным допуском
   до 80 px на изменение фокуса/масштаба IR-cut, но строго внутри своей рамки Pn.
...
объект за пределами допуска. Рекомендуемый допуск — 8–12 px. После успешной
Day↔Night-проверки площадь и форма ореола используются как мягкие веса, а не как
покадровые запреты. HOLD возникает только при отсутствии яркого локального ядра или
при выходе за координатный допуск. Причина печатается после HOLD/LOST:
LOW_SIGNAL, NO_CORE, CORE_AREA, MAX_JUMP или MAX_TRAVEL.
...
"""
    ...
    def _show_about(self):
        messagebox.showinfo(
            "О программе",
            f"Система обнаружения отражателей v{APP_VERSION}\n\n"
            ...
            "• крестики текущего и стартового центров вместо залитых точек\n"
            "• отдельный слой фактической яркой маски вычисления центра\n"
            ...
            "• Day↔Night-поиск строго внутри ручной рамки каждого Pn\n"
            "• запрет глобального fallback и переназначения между P1…Pn\n"
            "• проверка ромбовидного ИК-ореола внутри рамки\n"
            ...
            "• устойчивое Night-сопровождение без жесткой покадровой отбраковки формы\n"
            "• вывод причины HOLD/LOST в подписи каждой точки\n"
    ...
    def start_ir_flash_scan(self):
        ...
        if not self.calibration.peak_regions:
            messagebox.showwarning(
                "ИК-поиск",
                "Сначала задайте точные области P1…Pn. "
                "Во время Day↔Night каждый отражатель ищется "
                "только внутри своей рамки.",
            )
            return
        try:
        ...
        # Каждый новый запуск берет именно текущие ручные
        # рамки. Сохраненный якорь прошлого скана не имеет права
        # подменить новую границу Pn.
    ...
    def _extract_ir_candidates(
        ...
        for label in range(1, count):
            ...
            diamond_score = diamond_halo_score(component)
    ...
    def _refine_ir_candidate_on_night(
        ...
        candidate: Dict,
        region_index: int,
        allowed_region: Optional[Rect] = None,
    ) -> bool:
        """Уточняет центр только по локальному ядру response-ромба.

        При Day↔Night-калибровке allowed_region — жесткая маска:
        даже локальный уточнитель не видит яркие объекты за рамкой Pn.
        """
        ...
        if allowed_region is not None:
            region_x, region_y, region_w, region_h = allowed_region
            global_y, global_x = np.indices(gray.shape, dtype=np.int32)
            global_x += crop_x
            global_y += crop_y
            allowed_mask = (
                (global_x >= region_x)
                & (global_x < region_x + region_w)
                & (global_y >= region_y)
                & (global_y < region_y + region_h)
            )
            binary[~allowed_mask] = 0
        ...
        if allowed_mask is not None:
            # Closing может на один пиксель восстановить маску
            # за границей, поэтому обрезаем ее повторно.
            binary[~allowed_mask] = 0
        count, labels, stats, centroids = cv2.connectedComponentsWithStats(
            binary, connectivity=8
        )
    ...
    def _validate_ir_local_signature(
        ...
        if allowed_region is not None:
            # После механического переключения IR-cut меняются
            # фокус и масштаб. Глобальное совмещение не всегда
            # одинаково точно в P1 и P2. Поэтому черное Day-ядро
            # может сдвигаться до 80 px, но ищется все равно
            # СТРОГО внутри собственной рамки Pn.
            _, _, region_width, region_height = allowed_region
            regional_radius = min(
                80.0,
                max(24.0, 0.42 * float(np.hypot(region_width, region_height))),
            )
            day_match_radius = max(
                day_match_radius, int(round(regional_radius))
            )
            ...
            for shift_y in range(-day_match_radius, day_match_radius + 1):
                for shift_x in range(-day_match_radius, day_match_radius + 1):
                    ...
                    if allowed_region is not None:
                        ...
                        valid &= (
                            (shifted_x >= region_x)
                            & (shifted_x < region_x + region_w)
                            & (shifted_y >= region_y)
                            & (shifted_y < region_y + region_h)
                        )
    ...
    def _analyze_ir_flash_frames(
        ...
        # Day→Night-калибровка выполняется СТРОГО внутри ручных рамок.
        # Никакого расширения и резервного прохода по кадру: P1 получает
        # кандидатов только из P1, P2 — только из P2 и т.д. Расширение области
        # применяется позже исключительно при сопровождении подтвержденной
        # подвижной метки.
        reference_regions = (
        ...
        for region_index, region in enumerate(reference_regions):
            exact_region = clip_rect(region, response.shape)
            if exact_region is None:
                continue
            search_rects.append((region_index, exact_region))
        ...
        # Объединяем повторные фрагменты только ВНУТРИ одной и той же Pn.
        # Кандидаты соседних рамок никогда не объединяются.
        candidates = merge_ir_candidates(
            raw_candidates,
            bbox_key="bbox",
        ...
        for candidate in candidates:
            hints = set(candidate.get("region_hints", set()))
            if len(hints) != 1:
                candidate["rejection_reason"] = "FOREIGN_REGION"
                rejected_candidates.append(candidate)
                continue
            refinement_region = int(next(iter(hints)))
            if not 0 <= refinement_region < len(reference_regions):
                candidate["rejection_reason"] = "FOREIGN_REGION"
                rejected_candidates.append(candidate)
                continue
            allowed_region = reference_regions[refinement_region]
            ...
            tracking_x, tracking_y, tracking_w, tracking_h = candidate[
                "tracking_bbox"
            ]
            inside_region = (
                region_x <= center_x < region_x + region_w
                and region_y <= center_y < region_y + region_h
                and tracking_x >= region_x
                and tracking_y >= region_y
                and tracking_x + tracking_w <= region_x + region_w
                and tracking_y + tracking_h <= region_y + region_h
            )
            if not inside_region:
                candidate["rejection_reason"] = "OUTSIDE_REGION"
                rejected_candidates.append(candidate)
                continue
        ...
        matches = assign_ir_candidates_to_regions(
            reference_regions,
            candidates,
            strict_hints=True,
        )
        ...
        try:
            ...
            logger.debug("Не удалось построить ИК heatmap: %s", exc)
        if calibration.show_frames:
        ...
        for region_index, candidate in matches:
            ...
            if calibration.show_bright_contours:
                core_pixels = np.asarray(
                    candidate.get("_tracking_core_pixels", []),
                    dtype=np.int32,
                )
                if core_pixels.ndim == 2 and core_pixels.shape[0] >= 1:
                    core_mask = np.zeros(preview.shape[:2], dtype=np.uint8)
                    valid = (
                        (core_pixels[:, 0] >= 0)
                        & (core_pixels[:, 0] < core_mask.shape[1])
                        & (core_pixels[:, 1] >= 0)
                        & (core_pixels[:, 1] < core_mask.shape[0])
                    )
                    visible_pixels = core_pixels[valid]
                    if visible_pixels.size:
                        core_mask[
                            visible_pixels[:, 1], visible_pixels[:, 0]
                        ] = 255
            ...
            if calibration.show_displacement_lines:
                cv2.line(
                    preview,
                    origin_point,
                    (px, py),
    ...
    def _initialize_ir_tracking_models(self):
        ...
        for index, region in enumerate(self.calibration.peak_regions):
            ...
            if detection is None:
                # Даже если локальный Night-детектор не смог повторно
                # сегментировать ядро, сохраняем рисунок вокруг строгого
                # Day→Night-центра. Перезахват не останется без проверки
                # идентичности.
                template_size = 25
                template = normalized_reflector_patch(
                    night_frame, anchor, template_size
                )
                model["tracking_template_size"] = template_size
                model["tracking_template"] = template.tolist()
                model["tracking_center_offset_x"] = 0.0
                model["tracking_center_offset_y"] = 0.0
                # Сам Day↔Night-кандидат уже был уточнен по Night-ядру.
                # Нулевая поправка безопаснее ложного LOST:NO_OFFSET.
                model["tracking_offset_valid"] = True
                continue
            center_offset = (
                np.asarray(detection.position, dtype=np.float64)
                - np.asarray(anchor, dtype=np.float64)
            )
            maximum_offset = max(3.0, min(12.0, anchor_radius))
            if (
                ...
                model["tracking_center_offset_x"] = 0.0
                model["tracking_center_offset_y"] = 0.0
                # Не принимаем сомнительную большую поправку, но сохраняем
                # подтвержденный Day↔Night-центр рабочим с нулевой поправкой.
                model["tracking_offset_valid"] = True
                template_size = 25
                template = normalized_reflector_patch(
                    night_frame, anchor, template_size
                ...
                model["tracking_template"] = template.tolist()
                continue
            # Day→Night-центр уже подтвержден двумя переключениями и является
            # правильной координатой маркера. Night-детектор нужен только для
            # сопровождения; разницу его центра с Day↔Night сохраняем как
            # постоянную служебную поправку и НЕ переписываем saved_anchor.
            detector_center = (
                float(detection.position[0]),
                float(detection.position[1]),
            )
            model["signature_center_x"] = float(anchor[0])
            model["signature_center_y"] = float(anchor[1])
            ...
            template_size = 25
            template = normalized_reflector_patch(
                night_frame, detector_center, template_size
            )
            model["tracking_template_size"] = template_size
            model["tracking_template"] = template.tolist()
    ...
    def _finish_ir_flash_scan(self, token: int, result):
        ...
        self.calibration.ir_confirmed_models = {
            str(region_index + 1): {
                ...
                "tracking_center_offset_x": 0.0,
                "tracking_center_offset_y": 0.0,
                # candidate['position'] уже является уточненным Night-ядром.
                "tracking_offset_valid": True,
                "tracking_template_size": 25,
                "tracking_template": None,
            }
            for region_index, candidate in matches
        ...
        self.calibration.ir_model_version = IR_MODEL_VERSION
        if matches:
            ...
            if len(self.calibration.ir_reference_regions) == len(
                ...
                self.calibration.roi_region = bounding_rect(regions)
    ...
    def _update_ir_settings_from_ui(self):
        ...
        self.calibration._enforce_ir_region_invariants()
        ...
        self.ir_strict_regions_var.set(True)
        self.ir_global_fallback_var.set(False)
    ...
    def _on_ir_lock_change(self, event=None):
        ...
        self.ir_lock_radius_var.set(self.calibration.ir_lock_radius)
        self.ir_max_travel_var.set(self.calibration.ir_max_travel)
        self.ir_search_scale_var.set(1.0)
        self.ir_strict_regions_var.set(True)
        self.ir_global_fallback_var.set(False)
        if self.tracker:
            self._reset_processing_modules(announce=False)
        ...
        self.status_label.config(
            text=(
                f"Удержание ИК-метки {state}, "
                "Day↔Night строго внутри своей Pn; "
    ...
    def _on_display_layer_change(self):
        ...
        for name in BASE_DISPLAY_SETTINGS:
            variable = getattr(self, f"{name}_var")
            setattr(self.calibration, name, bool(variable.get()))
        if hasattr(self, "status_label"):
            self.status_label.config(text="Слои отображения обновлены")

    def _global_detection_settings(self) -> Dict:
        if not self._update_calibration_from_ui():
            raise ValueError("Проверьте числовые параметры обнаружения")
        return {
    ...
    def _update_calibration_from_ui(self):
        try:
            ...
            self.calibration.ir_stationary_deadband = float(
                np.clip(self.stationary_deadband_var.get(), 0.0, 2.0)
            )
            self.calibration.ir_reacquire_confirm_frames = int(
                np.clip(self.reacquire_frames_var.get(), 1, 10)
            )
            self.calibration.ir_reacquire_consistency = float(
                np.clip(self.reacquire_consistency_var.get(), 1.0, 20.0)
            )
            self.calibration.ir_template_min_score = float(
                np.clip(self.template_min_score_var.get(), 0.0, 0.95)
            )
    ...
    def _update_ui_from_calibration(self):
        ...
        self.stationary_deadband_var.set(
            self.calibration.ir_stationary_deadband
        )
        self.reacquire_frames_var.set(
            self.calibration.ir_reacquire_confirm_frames
        )
        self.reacquire_consistency_var.set(
            self.calibration.ir_reacquire_consistency
        )
        self.template_min_score_var.set(
            self.calibration.ir_template_min_score
        )
        if hasattr(self, "show_points_var"):
            for name in BASE_DISPLAY_SETTINGS:
                variable = getattr(self, f"{name}_var")
                variable.set(bool(getattr(self.calibration, name)))
        if hasattr(self, "ir_settle_var"):
            self.calibration._enforce_ir_region_invariants()
            self.ir_settle_var.set(self.calibration.ir_settle_seconds)
            self.ir_flash_delta_var.set(self.calibration.ir_flash_delta)
    ...
    def _current_preset_parameters(self) -> Dict:
        if not self._update_calibration_from_ui():
            raise ValueError("Проверьте числовые параметры перед сохранением пресета")
        return {
    ...
    def save_calibration(self):
        ...
        if filename:
            if not self._update_calibration_from_ui():
                messagebox.showerror(
                    "Ошибка параметров",
                    "Проверьте числовые параметры перед сохранением.",
                )
                return
            try:
    ...
    def _connect_rtsp(self) -> bool:
        ...
        if self.rtsp_camera.connect():
            ...
            return True
        error_detail = self.rtsp_camera.last_error
        self.rtsp_camera = None
        self.connection_status.config(text="Ошибка RTSP", foreground="red")
        messagebox.showerror(
            "Ошибка подключения",
            "Не удалось подключиться к RTSP. "
            "Проверьте URL, сеть, логин, пароль и FFmpeg."
    ...
    def _start_detection(self):
        ...
        if not self._update_calibration_from_ui():
            messagebox.showerror(
                "Ошибка параметров",
                "Проверьте числовые параметры перед запуском отслеживания.",
            )
            return
        self.tracker = ReflectorTracker(self.calibration)
        self.tracker.detector.background = self.background_frame
    ...
    def toggle_calibration(self):
        if not self.is_calibrating:
            ...
            if not self._update_calibration_from_ui():
                messagebox.showerror(
                    "Ошибка параметров",
                    "Проверьте числовые параметры перед калибровкой.",
                )
                return
            if self.tracker is None:
                self.tracker = ReflectorTracker(self.calibration)
    ...
    def select_peak_regions(self):
        ...
        if not self._update_calibration_from_ui():
            messagebox.showerror(
                "Ошибка параметров",
                "Проверьте числовые параметры перед выбором областей.",
            )
            return
    ...
    def _finish_peak_region_selection(self):
        ...
        self.calibration.roi_region = bounding_rect(selected)
        if self.tracker:
            self._reset_processing_modules(announce=False)
        self._update_region_status()
        self.status_label.config(
    ...
    def _update_display(self):
        ...
        try:
            rtsp_connected = bool(
                self.rtsp_camera is not None and self.rtsp_camera.is_connected
            )
            connected = rtsp_connected or self.local_cap is not None
            if (
                self.use_rtsp
                and self.rtsp_camera is not None
                and not rtsp_connected
                and (self.is_running or self.is_calibrating)
            ):
                self.connection_status.config(
                    text="RTSP потерян", foreground="red"
                )
                detail = self.rtsp_camera.last_error or "поток остановлен"
                self.status_label.config(text=f"RTSP потерян: {detail}")
        ...
        processing_ms = (time.perf_counter() - loop_started) * 1000.0
        delay_ms = max(1, int(round(30.0 - processing_ms)))
        self.root.after(delay_ms, self._update_display)
