...
class ReflectorDetector:
    ...
    def detect_in_region(
        ...
        if anchor_probe_local is not None:
            ...
            # сжаться до 2–4 px. После двух циклов Day↔Night это
        ...
        for label in range(1, count):
            ...
            if anchor_local is not None:
                ...
                score = (
                    0.58 * anchor_proximity
                    + 0.16 * proximity
                    + 0.10 * peak_similarity
                    + 0.08 * area_similarity
                    + 0.08 * diamond_similarity
                )
            else:
...
class ReflectorApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Система устойчивого отслеживания отражателей v5.26")
...
"""
    ...
    def _show_about(self):
        messagebox.showinfo(
            "О программе",
            "Система обнаружения отражателей v5.26\n\n"
    ...
    def _finish_ir_flash_scan(self, token: int, result):
        ...
        self.calibration.ir_model_version = 12
        if matches:
