...
def assign_ir_candidates_to_regions(
    ...
    for candidate_index, candidate in enumerate(candidates):
        ...
        for region_index in range(len(reference_regions)):
            ...
            # На реальных кадрах край откоса получил D=0.77, но S=0.48,
            # тогда как настоящий P2 имел D=0.50 и S=0.57. Поэтому D и длина
            # не имеют права перекрыть более сильный переход Day→Night.
            line_weight = (
                0.90 * signature
                + 0.06 * tracking_diamond
                + 0.04 * length_score
            )
            score = (
                0.96 * line_weight
                + 0.025 * local_evidence
                + 0.010 * quality
                + 0.005 * inside
            )
            route_diagnostics[str(region_index + 1)] = {
...
class ReflectorApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Система устойчивого отслеживания отражателей v5.21")
...
"""
    ...
    def _show_about(self):
        messagebox.showinfo(
            "О программе",
            "Система обнаружения отражателей v5.21\n\n"
    ...
    def _validate_ir_local_signature(
        ...
        strong_core_transition = core_gain >= 0.23 or core_gain_high >= 0.30
        local_transition = (
        ...
        signature_score = float(
            np.clip(
                0.38 * np.clip(core_gain_high / 0.60, 0.0, 1.0)
                + 0.22 * np.clip(night_core / 0.95, 0.0, 1.0)
                + 0.16 * np.clip(night_contrast / 0.45, 0.0, 1.0)
                + 0.12 * np.clip(contrast_gain / 0.45, 0.0, 1.0)
                + 0.06 * np.clip((0.82 - day_core) / 0.62, 0.0, 1.0)
                + 0.06
                * np.clip(
    ...
    def _finish_ir_flash_scan(self, token: int, result):
        ...
        self.calibration.ir_model_version = 7
        if matches:
