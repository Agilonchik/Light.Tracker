...
class ReflectorApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Система устойчивого отслеживания отражателей v5.23")
...
"""
    ...
    def _show_about(self):
        messagebox.showinfo(
            "О программе",
            "Система обнаружения отражателей v5.23\n\n"
    ...
    def _validate_ir_local_signature(
        ...
        def shifted_black_core_level(image: np.ndarray) -> float:
            """Ищет совпадающее черное ядро в пределах ошибки совмещения."""
            scale = image_scale(image)
            darkest_level = np.inf
            for shift_y in range(-2, 3):
                for shift_x in range(-2, 3):
                    ...
                    darkest_level = min(darkest_level, level)
            return float(darkest_level)
        ...
        for cycle_index in range(raw_cycle_count):
            ...
            if day_image.shape != day_normalized.shape or night_image.shape != day_normalized.shape:
                continue
            day_level = shifted_black_core_level(day_image)
    ...
    def _finish_ir_flash_scan(self, token: int, result):
        ...
        self.calibration.ir_model_version = 9
        if matches:
