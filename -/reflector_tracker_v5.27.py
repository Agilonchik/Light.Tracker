...
class ReflectorApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Система устойчивого отслеживания отражателей v5.27")
...
"""
    ...
    def _show_about(self):
        messagebox.showinfo(
            "О программе",
            "Система обнаружения отражателей v5.27\n\n"
    ...
    def _initialize_ir_tracking_models(self):
        ...
        for index, region in enumerate(self.calibration.peak_regions):
            ...
            if (
                ...
                model["tracking_template"] = template.tolist()
                continue
            # Day→Night нашел нужный объект, а обычный Night-детектор уточнил
            # геометрический центр его компактного яркого ядра. Именно этот
            # центр становится стартовой координатой, а не приблизительный
            # центр response-ореола.
            refined_center = (
            ...
            self.calibration.ir_confirmed_centers[point_id] = [
                refined_center[0],
                refined_center[1],
            ]
            ...
            template_size = 25
            template = normalized_reflector_patch(
                night_frame, refined_center, template_size
            )
            model["tracking_template_size"] = template_size
            model["tracking_template"] = template.tolist()
    ...
    def _finish_ir_flash_scan(self, token: int, result):
        ...
        self.calibration.ir_model_version = 13
        if matches:
