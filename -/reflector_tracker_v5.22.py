...
class ReflectorApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Система устойчивого отслеживания отражателей v5.22")
...
"""
    ...
    def _show_about(self):
        messagebox.showinfo(
            "О программе",
            "Система обнаружения отражателей v5.22\n\n"
    ...
    def _validate_ir_local_signature(
        ...
        def shifted_day_level(image: np.ndarray) -> float:
            """Ищет тот же статичный объект в пределах ошибки совмещения."""
            ...
            for shift_y in range(-2, 3):
                for shift_x in range(-2, 3):
                    ...
                    if np.count_nonzero(valid) < 3:
                        continue
                    level = float(
                        np.median(image[shifted_y[valid], shifted_x[valid]])
                        / scale
                    )
        ...
        for cycle_index in range(raw_cycle_count):
            ...
            if day_image.shape != day_normalized.shape or night_image.shape != day_normalized.shape:
                continue
            day_level = shifted_day_level(day_image)
        ...
        if has_repeated_raw_check:
            ...
            # прирост яркости. Поиск Day-уровня со сдвигом ±2 px отбрасывает
            # статичную деталь даже при небольшой ошибке регистрации кадров.
            if raw_gain_min < 0.14:
                candidate["signature_failure"] = "SIG_STATIC"
                return False
            if raw_day_level > 0.70 and raw_gain_min < 0.24:
                candidate["signature_failure"] = "SIG_DAY"
                return False
        else:
            # Совместимость со старыми тестами/вызовами. Живой поиск v5.22
    ...
    def _analyze_ir_flash_frames(
        ...
        def flash_response(day_image, night_image, day_raw, night_raw):
            ...
            raw_day_dark = np.clip((0.72 - day_raw_unit) / 0.52, 0.0, 1.0)
    ...
    def _finish_ir_flash_scan(self, token: int, result):
        ...
        self.calibration.ir_model_version = 8
        if matches:
