...
class ReflectorApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Система устойчивого отслеживания отражателей v5.31")
...
"""
    ...
    def _show_about(self):
        messagebox.showinfo(
            "О программе",
            "Система обнаружения отражателей v5.31\n\n"
    ...
    def _analyze_ir_flash_frames(
        ...
        for candidate in sorted(
            ...
            for existing in candidates:
                if set(candidate.get("region_hints", set())).isdisjoint(
    ...
    def _finish_ir_flash_scan(self, token: int, result):
        ...
        self.calibration.ir_model_version = 14
        if matches:
    ...
    def _update_ir_settings_from_ui(self):
        ...
        self.calibration.ir_search_scale = 1.0
        self.calibration.ir_sample_count = int(np.clip(sample_count, 5, 9))
        self.calibration.ir_strict_regions = True
        self.calibration.ir_global_fallback = False
    ...
    def _on_display_layer_change(self):
        ...
        self.calibration.show_start_points = bool(
            self.show_start_points_var.get()
        )
        self.calibration.show_labels = bool(self.show_labels_var.get())
        self.calibration.show_bright_contours = bool(
            self.show_bright_contours_var.get()
        )
        ...
        self.calibration.show_displacement_lines = bool(
            self.show_displacement_lines_var.get()
        )
        self.calibration.show_displacements = bool(
            self.show_displacements_var.get()
        )
        self.calibration.show_summary = bool(self.show_summary_var.get())
    ...
    def _update_ui_from_calibration(self):
        ...
        if hasattr(self, "show_points_var"):
            ...
            self.show_start_points_var.set(self.calibration.show_start_points)
            self.show_labels_var.set(self.calibration.show_labels)
            self.show_bright_contours_var.set(
                self.calibration.show_bright_contours
            )
            ...
            self.show_displacement_lines_var.set(
                self.calibration.show_displacement_lines
            )
            self.show_displacements_var.set(
                self.calibration.show_displacements
            )
            self.show_summary_var.set(self.calibration.show_summary)
